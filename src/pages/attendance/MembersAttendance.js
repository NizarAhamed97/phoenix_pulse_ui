...
const MemberAttendance = () => {
  ...
  const handleCheckIn = async () => {
    if (!memberID) return;
    try {
      await axios.post("http://localhost:3000/api/attendance/members/checkin", { FK_MemberID: memberID });
      setMemberID("");
      fetchAttendance();
    } catch (error) {
  ...
  const fetchAttendance = async () => {
    try {
      const presentRes = await axios.get("http://localhost:3000/api/attendance/members/present");
      console.log(presentRes)
      const absentRes = await axios.get("http://localhost:3000/api/attendance/members/absent");
